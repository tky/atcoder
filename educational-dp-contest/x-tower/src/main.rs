...
fn resolve(blocks: &mut Vec<Block>) -> u64 {
    ...
    //   |
    //   A
    // これが成立している場合、X + Wb <= Saが成立してる。つまり X <= Sa - Wb
    // このAとBを入れ替えたとする。この時Bが壊れないためには
    // X + Wa <= Sb である必要がある。つまり X <= Sb - Wa
    // 前提: X <= Sa - Wb
    // 示したいこと: X <= Sb - Wa
    // もし Sa -Wb <= Sb - Waであれば
    // X <= Sa - Wb <= Sb- Waとなり示したいことを実現できる
    // よって、AとBは
    // Sa - Wb <= Sb - Wa であれば入れ替え可能
    // 式を変形して
    // Sa + Wa <= Sb + Wb
    ...
    for block in blocks.iter() {
        ...
        for j in (0..=s).rev() {
            if j + w <= max_weight {
                // ループの条件がj <= s なので、現在のブロックは重さ j の塔を支えられる。
                // 重さ合計が j の塔の一番下に、現在のブロックを置く。
                // すると新しい塔の重さは j + w、価値は dp[j] + v になる。
